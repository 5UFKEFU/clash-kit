...
//export proxyclash_CPUInfo_ModelName_Set
func proxyclash_CPUInfo_ModelName_Set(refnum C.int32_t, v C.nstring) {
	ref := _seq.FromRefNum(int32(refnum))
	_v := decodeString(v)
	ref.Get().(*clash.CPUInfo).ModelName = _v
}

//export proxyclash_CPUInfo_ModelName_Get
func proxyclash_CPUInfo_ModelName_Get(refnum C.int32_t) C.nstring {
	ref := _seq.FromRefNum(int32(refnum))
	v := ref.Get().(*clash.CPUInfo).ModelName
	_v := encodeString(v)
	return _v
}

//export proxyclash_CPUInfo_Cores_Set
func proxyclash_CPUInfo_Cores_Set(refnum C.int32_t, v C.nint) {
	ref := _seq.FromRefNum(int32(refnum))
	_v := int(v)
	ref.Get().(*clash.CPUInfo).Cores = _v
}

//export proxyclash_CPUInfo_Cores_Get
func proxyclash_CPUInfo_Cores_Get(refnum C.int32_t) C.nint {
	ref := _seq.FromRefNum(int32(refnum))
	v := ref.Get().(*clash.CPUInfo).Cores
	_v := C.nint(v)
	return _v
}

//export new_clash_CPUInfo
func new_clash_CPUInfo() C.int32_t {
	return C.int32_t(_seq.ToRefNum(new(clash.CPUInfo)))
}
...
// skipped function GetCPUInfo with unsupported parameter or result types
//
